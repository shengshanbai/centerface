...
def mobilenetv2_10(pretrained=True, width_mult=1.0):
...
def fill_up_weights(up):#中心往外递减的分部
    w = up.weight.data
...
class CenterFaceNet(nn.Module):
    def __init__(self,width_mult=1.0,feature_channel=24):
        super().__init__()
        self.base=mobilenetv2_10(width_mult=width_mult)
        channels=self.base.feat_channel
        self.dla_up = MobileNetUp(channels,feature_channel)
        ...
        self.center_off_conv=nn.Conv2d(feature_channel, 2,
                              kernel_size=1, stride=1,
                              padding=0, bias=True)
        self.wh_conv=nn.Conv2d(feature_channel, 2,
                              kernel_size=1, stride=1,
                              padding=0, bias=True)
        self.landmarks_conv=nn.Conv2d(feature_channel,10,
                              kernel_size=1, stride=1,
                              padding=0, bias=True)
    ...
    def forward(self, x):
        x = self.base(x)
        x = self.dla_up(x)
        heat_map=self.heat_conv(x)
        center_off=self.center_off_conv(x)
        wh=self.wh_conv(x)
        landmarks=self.landmarks_conv(x)
        return heat_map, center_off, wh, landmarks
