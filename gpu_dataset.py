...
def Data_anchor_sample(image, face_boxes, face_keys=None):
    ...
    image = ndimage.zoom(image, zoom=(ratio, ratio, 1.0), order=1, mode='opencv')
...
def grayscale(image):
    height, width, _ = image.shape
    gray_image = cupy.zeros((height, width, 1), dtype=cupy.float32)
    gray_image[:, :, 0] = 0.07 * image[:, :, 0] + 0.72 * image[:, :, 1] + 0.21 * image[:, :, 2]
    return gray_image


def lighting_(data_rng, image, alphastd, eigval, eigvec):
    alpha = data_rng.normal(scale=alphastd, size=(3,))
    image += cupy.dot(eigvec, eigval * alpha)
...
def draw_gaussian(heatmap, center, radius, k=1):
    ...
    if min(masked_gaussian.shape) > 0 and min(masked_heatmap.shape) > 0:  # 取重叠的最大值
        cupy.maximum(masked_heatmap, masked_gaussian * k, out=masked_heatmap)
    return heatmap
...
def affine_transform(pts, aff_mat):
    ...
    hom_pts = cupy.ones((trans_pts.shape[0] + 1, trans_pts.shape[1]), trans_pts.dtype)
    hom_pts[0:2, :] = trans_pts
    trans_out = cupy.dot(aff_mat, hom_pts)
...
class WFaceDB(data.Dataset):
    def __init__(self, lmdb_dir, is_train, shuffle=True):
        ...
        self.eig_val = cupy.array([0.2141788, 0.01817699, 0.00341571],
                                  dtype=cupy.float32)
        self.eig_vec = cupy.array([
            [-0.58752847, -0.69563484, 0.41340352],
            [-0.5832747, 0.00994535, -0.81221408],
            [-0.56089297, 0.71832671, 0.41158938]
        ], dtype=cupy.float32)
        self.mean = cupy.array([0.40789654, 0.44719302, 0.47026115],
                             dtype=cupy.float32).reshape(1, 1, 3)
        self.std = cupy.array([0.28863828, 0.27408164, 0.27809835],
                            dtype=cupy.float32).reshape(1, 1, 3)
        self.data_rng = cupy.random.RandomState(123)
    ...
    def __getitem__(self, index):
        ...
        trans_input[:, [0, 1]] = trans_input[:, [1, 0]]
        input_image = cupy.zeros((g_config.input_res, g_config.input_res, channel), dtype=cupy.float32)
        for i in range(channel):
            ndimage.affine_transform(image[:, :, i], trans_input,order=1,output_shape=(g_config.input_res, g_config.input_res),
        ...
        if has_face:
            ...
            for i in range(num_faces):
                ...
                if w > 0 and h > 0:
                    ...
                    # 人脸中心点
                    face_center = cupy.array([(boxes[i, 0] + boxes[i, 2]) / 2, (boxes[i, 1] + boxes[i, 3]) / 2],
                                           dtype=cupy.float32)
...
def wface_collate_fn(batch):
    ...
    for i in range(data_class):
        if i <= 1:  # 图像数据
            c_list = [from_dlpack(toDlpack(row[i])) for row in batch]
            ...
            for k in range(len(batch)):
                data = from_dlpack(toDlpack(batch[k][i]))
                or_shape = data.shape
...
if __name__ == "__main__":
    reader = WFaceDB(r'C:\temp\WIDER_FACE\train_lmdb', True)
    for i in range(len(reader)):
        ...
        heat_map =cupy.asnumpy(data_item[1])
        ...
        face_mask = cupy.asnumpy(data_item[5])
        for j in range(len(face_mask)):
            if face_mask[j] > 0:
                ...
                wh = np.exp(cupy.asnumpy(data_item[2][j]))
                face_off = cupy.asnumpy(data_item[4][j])
                ...
                face_key_off = cupy.asnumpy(data_item[6])
                face_key_mask = cupy.asnumpy(data_item[7])
                for k in range(4):
